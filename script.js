...
const galleryImages = [
    { id: 1, category: 'nature', title: 'Mountain Sunset', src: 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=800', archived: false },
    { id: 2, category: 'nature', title: 'Forest Path', src: 'https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=800', archived: false },
    { id: 3, category: 'tech', title: 'Coding Setup', src: 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?w=800', archived: false },
    { id: 4, category: 'tech', title: 'Circuit Board', src: 'https://images.unsplash.com/photo-1518770660439-4636190af475?w=800', archived: false },
    { id: 5, category: 'abstract', title: 'Color Waves', src: 'https://images.unsplash.com/photo-1557672172-298e090bd0f1?w=800', archived: false },
    { id: 6, category: 'abstract', title: 'Geometric Art', src: 'https://images.unsplash.com/photo-1550684848-fac1c5b4e853?w=800', archived: false },
    { id: 7, category: 'nature', title: 'Ocean View', src: 'https://images.unsplash.com/photo-1505142468610-359e7d316be0?w=800', archived: false },
    { id: 8, category: 'tech', title: 'Data Visualization', src: 'https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=800', archived: false },
    { id: 9, category: 'abstract', title: 'Light Patterns', src: 'https://images.unsplash.com/photo-1558591710-4b4a1ae0f04d?w=800', archived: false },
    { id: 10, category: 'nature', title: 'Desert Landscape', src: 'https://images.unsplash.com/photo-1509316785289-025f5b846b35?w=800', archived: false },
    { id: 11, category: 'tech', title: 'Futuristic Design', src: 'https://images.unsplash.com/photo-1451187580459-43490279c0fa?w=800', archived: false },
    { id: 12, category: 'abstract', title: 'Digital Art', src: 'https://images.unsplash.com/photo-1567095761054-7a02e69e5c43?w=800', archived: false }
];
...
function displayGalleryImages() {
    ...
    const activeImages = galleryImages.filter(img => !img.archived);
    
    if (activeImages.length === 0) {
        gallery.innerHTML = '<p style="color: var(--text-muted); text-align: center; padding: 2rem; grid-column: 1/-1;">No active photos. Check archived section.</p>';
        return;
    }
        ...
        item.innerHTML = `
            ...
            <div class="gallery-overlay">
                ...
                <div class="gallery-actions">
                    <button class="gallery-action-btn view-btn" onclick="event.stopPropagation(); viewImageById(${img.id})" title="View">
                        👁️ View
                    </button>
                    <button class="gallery-action-btn archive-btn" onclick="event.stopPropagation(); archiveImage(${img.id})" title="Archive">
                        📁 Archive
                    </button>
                    <button class="gallery-action-btn delete-btn" onclick="event.stopPropagation(); deleteImage(${img.id})" title="Delete">
                        🗑️ Delete
                    </button>
                </div>
            </div>
...
function displayArchivedImages() {
    const archivedGallery = document.getElementById('archived-gallery');
    if (!archivedGallery) return;
    
    const archivedImages = galleryImages.filter(img => img.archived);
    
    if (archivedImages.length === 0) {
        archivedGallery.innerHTML = '<p style="color: var(--text-muted); text-align: center; padding: 2rem; grid-column: 1/-1;">No archived photos yet.</p>';
        return;
    }
    
    archivedGallery.innerHTML = '';
    ...
    archivedImages.forEach((img) => {
        ...
        item.setAttribute('data-category', img.category);
        item.innerHTML = `
            <img src="${img.src}" alt="${img.title}" loading="lazy" style="width: 100%; height: 100%; object-fit: cover; opacity: 0.7;">
            <div class="gallery-overlay">
                <h4>${img.title} (Archived)</h4>
                <p>${img.category}</p>
                <div class="gallery-actions">
                    <button class="gallery-action-btn view-btn" onclick="event.stopPropagation(); viewImageById(${img.id})" title="View">
                        👁️ View
                    </button>
                    <button class="gallery-action-btn restore-btn" onclick="event.stopPropagation(); restoreImage(${img.id})" title="Restore">
                        ↩️ Restore
                    </button>
                    <button class="gallery-action-btn delete-btn" onclick="event.stopPropagation(); deleteImage(${img.id})" title="Delete">
                        🗑️ Delete
        ...
        `;
        archivedGallery.appendChild(item);
    });
}
...
function archiveImage(id) {
    ...
    if (image) {
        ...
        displayGalleryImages();
        displayArchivedImages();
        showNotification(`"${image.title}" archived successfully!`, 'info');
...
function restoreImage(id) {
    ...
    if (image) {
        ...
        displayGalleryImages();
        displayArchivedImages();
        showNotification(`"${image.title}" restored successfully!`, 'success');
    }
}
...
function deleteImage(id) {
    ...
    if (!confirm(`Are you sure you want to permanently delete "${image.title}"?`)) return;
    ...
    displayGalleryImages();
    displayArchivedImages();
    showNotification(`"${image.title}" deleted permanently!`, 'error');
}
...
// Add these event listeners to your student form inputs

document.addEventListener('DOMContentLoaded', function() {
  const studentIdInput = document.getElementById('student-id');
  const contactInput = document.getElementById('student-contact');
  const firstNameInput = document.getElementById('student-fname');
  const lastNameInput = document.getElementById('student-lname');
  const middleInitialInput = document.getElementById('student-mname');
  ...
  // Middle Initial - only allow single letter
  if (middleInitialInput) {
    middleInitialInput.addEventListener('input', function(e) {
      // Remove numbers and special characters, keep only letters, limit to 1
  ...
  // Contact - only allow numbers, max 11 digits
  if (contactInput) {
...
let fullExpression = ''; // NEW: Track the full expression
...
function updateDisplay() {
    ...
    // Update main display - show empty if value is '0' and operator was just pressed
    if (currentValue === '0' && operatorJustPressed) {
        display.textContent = '';
    } else {
        display.textContent = currentValue;
    }
    
    // Update expression display - show full expression
    if (expression) {
        if (fullExpression) {
...
function inputNumber(num) {
    ...
    if (shouldResetScreen || operatorJustPressed) {
        currentValue = num;
...
function selectOperator(operator) {
    ...
    // If operator was just pressed and user presses another operator, just change the operator
    if (operatorJustPressed && previousValue) {
        currentOperator = operator;
        // Update the last operator in full expression
    ...
    if (fullExpression === '') {
        fullExpression = currentValue + ' ' + operator;
    } else {
        // Only add current value if it's not '0' (to prevent duplication after %)
        if (currentValue !== '0' || !operatorJustPressed) {
            fullExpression += ' ' + currentValue + ' ' + operator;
        } else {
            // Just add the operator (after % was pressed)
    ...
    // Set to '0' which will display as empty
    currentValue = '0';
...
function compute() {
    ...
    // Check if user pressed = without entering second number
    if (operatorJustPressed && currentValue === '0') {
        showNotification('Please enter a second number', 'error');
        currentValue = 'Error';
        previousValue = '';
        currentOperator = '';
        fullExpression = '';
    ...
    let result = calculateExpression(fullExpression);
...
function calculateExpression(expr) {
    ...
    // Check if first number has percentage
    if (tokens[1] === '%') {
        result = result / 100;
        // Start from index 2 for next operations
        var startIndex = 2;
    } else {
        var startIndex = 1;
    }
    ...
    for (let i = startIndex; i < tokens.length; i += 2) {
        ...
        // Check if this number has a percentage after it
        if (tokens[i + 2] === '%') {
            nextNum = nextNum / 100;
            i++; // Skip the % symbol in next iteration
        }
...
function percentage() {
    ...
    // Clear current value to prevent it from being added again
    currentValue = '0';
    shouldResetScreen = true;
    operatorJustPressed = true; // Set this flag so next operator doesn't duplicate the number
    updateDisplay();
}
